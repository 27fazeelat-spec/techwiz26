...
ITEM_STATUS = {
    "Verified": ("Verified", "Every check passed: the item matches its policy and the approved list of who learns what."),
    ...
    "Requirement Missing": ("Missing from the plan", "The approved list of who learns what says this person must learn this rule, but the plan the AI wrote leaves it out."),
    "Unsupported Requirement": ("Not required for this role", "The plan includes a rule that the approved list does not give to this person's job or situation."),
    ...
    "Manual Review Required": ("Needs a person", "The app could not decide on its own; a reviewer has to look at it."),
}

PLAN_STATUS = {
    "Verified": ("Verified", "Every must-learn rule is in the plan, and every item points to the policy it comes from."),
    "Verified with Warning": ("Verified, with notes", "Every must-learn rule is in the plan; a few small notes are worth a look before giving it out."),
    "Incomplete": ("Incomplete", "Some must-learn rules for this job are missing from the plan."),
    ...
    "Failed": ("Could not be written", "The AI service did not return a plan, so nothing was checked or given out."),
    "generating": ("Generating", "The plan is being written and checked."),
    "superseded": ("Replaced", "A newer version of this plan exists."),
}

DOC_STATUS = {
    "active": ("In force", "In force: training is taught from it."),
    "superseded": ("Older version", "A newer version replaced it; kept for history only."),
    "scheduled": ("Coming soon", "Approved, but it only starts on a later date."),
    "expired": ("Expired", "Past its review date with no new version yet; training from it is flagged."),
    "draft": ("Draft", "Not approved yet; nothing is taught from it."),
    "pending": ("Processing", "Being processed."),
}

GLOSSARY = {
    "requirement": "A single rule taken from a policy, such as \"Delete passport scans within 7 days\". The app finds them from words like must and shall.",
    "matrix": "Who learns what: the list of which rules each job must learn, and by when. A person approves it, and every plan is checked against it.",
    ...
    "coverage": "Of the must-learn rules for this person's job, how many the plan actually teaches.",
    "traceability": "How many plan items point to a real, current policy section that supports them.",
    "consistency": "How closely the AI's choices match the approved list of who learns what: must or should, when it is due, which policy, and so on.",
    "review": "Approvals: the parts of a plan the app could not confirm, waiting for a person to approve, fix or rewrite them.",
...
_RELATED = re.compile(r" and (\d+) related areas?$")


def module_title(title):
    """Older plans named merged modules "X and 2 related areas"; show that as "X +2 more topics"."""
    m = _RELATED.search(title or "")
    if not m:
        return title
    n = int(m.group(1))
